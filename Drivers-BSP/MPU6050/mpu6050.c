...
#define MPU_INT_PORT GPIOG
#define MPU_INT_PIN GPIO_PIN_5
#define MPU6050_INT_ENABLE __HAL_RCC_GPIOG_CLK_ENABLE()

#define MPU6050_CLK_ENABLE __HAL_RCC_GPIOG_CLK_ENABLE()
...
u8 MPU_Read_Byte(u8 reg)
{
	...
	IIC_Wait_Ack(&MPU_bus);
	res = IIC_Read_Byte(&MPU_bus);
	IIC_Send_NotAck(&MPU_bus);
	IIC_Stop(&MPU_bus);
	return res;
}
